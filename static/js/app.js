...
async function loadCameraLimits() {
  try {
    const [configResp, camerasResp, storageResp] = await Promise.all([
      ...
      fetch(apiUrl('api/storage/cameras')),
    ]);
...
async function loadAlarmStatus() {
  try {
    const resp = await fetch(apiUrl('api/alarm/status'));
    const data = await resp.json();
...
async function loadAlarmLog() {
  try {
    const resp = await fetch(apiUrl('api/alarm/log?limit=100'));
    const data = await resp.json();
...
async function clearAlarmLog() {
  try {
    await fetch(apiUrl('api/alarm/clear'), { method: 'POST' });
    loadAlarmLog();
  } catch (err) {
...
async function loadGo2RTCStatus() {
  try {
    const resp = await fetch(apiUrl('api/go2rtc/status'));
    if (!resp.ok) return;
    const data = await resp.json();
...
async function loadCameras() {
  try {
    const resp = await fetch(apiUrl('api/go2rtc/cameras'));
    if (!resp.ok) return;
...
async function reorderCamera(from, to) {
  try {
    const resp = await fetch(apiUrl('api/go2rtc/reorder'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function restartGo2RTC() {
  ...
  try {
    const resp = await fetch(apiUrl('api/go2rtc/restart'), { method: 'POST' });
    if (!resp.ok) {
...
async function runGo2RTCInstall(url, modal) {
  ...
  try {
    ...
    const resp = await fetch(apiUrl('api/go2rtc/install'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function runGo2RTCUpdate(url, modal) {
  ...
  try {
    ...
    const resp = await fetch(apiUrl('api/go2rtc/update'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url }),
    });
